...
class CreateSim(object):
    ...
    def init_particles(self):
        ...
        rand_x = np.random.normal(self.x_gt[0], 1, self.num_particles)
        rand_y = np.random.normal(self.x_gt[1], 1, self.num_particles)
        rand_angle = np.random.normal(self.x_gt[2], 1, self.num_particles)
    ...
    while not sim.done and iteration < max_iters:
        if sim.plot:
            if iteration % 6 == 0:
                ...
                print [round(sim.x_t[0][0], 2), round(sim.x_t[1][0], 2)]
                print "Actual position = "
                print [round(sim.x_gt[0][0], 2), round(sim.x_gt[1][0], 2)]
                #sim.print_particles()
